...
problem.inputs.uConstraintTol=[1e-1, 1e-1, 1e-1, 1e-1, 1e-1, 1e-1];
...
problem.constraints.gTol_eq=[0.05];
...
stageCost=ct(1:size(t,1)).*u_B_grid-0.98*ct(1:size(t,1)).*u_s_grid+Price_carbon*c_em(1:size(t,1)).*u_B_grid.*delt; 
