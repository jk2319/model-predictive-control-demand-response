...
plot([solution.T(1),solution.T(end)],-problem.inputs.uu(4)*ones(1,2),'k--','LineWidth',2)
...
plot([solution.T(1),solution.T(end)],problem.inputs.uu(6)*ones(1,2),'b--','LineWidth',2)
plot([solution.T(1),solution.T(end)],problem.inputs.uu(7)*ones(1,2),'g--','LineWidth',2)
xlim([0 solution.T(end)])
xlabel('Time [h]')
ylabel('Power kW')
legend('Power from Grid (Kw)','Sold power (kw)')
...
legend( 'Power-PV (Kw)','Power from Grid (Kw)',  'Power-Battery-discharge (Kw)','Sold power (kw)')
...
xlim([0 simtime+solution.tf])
xlabel('Time [h]')
ylabel('Power, Load, Sold power kW')
legend( 'Load Demand (Kw)','Sold power (kw)','Bought power (kw)')
