...
legend('Room Temperature ','External Temperature ')
grid minor; box on;
ax = gca;
...
plot([T(1),T(end)],problem.states.xu(2)*ones(1,2),'b--','LineWidth',2)
plot([T(1),T(end)],problem.states.xl(2)*ones(1,2),'b--','LineWidth',2)
% plot([T(1),T(end)],problem.inputs.ul(4)*ones(1,2),'k--')
plot([T(1),T(end)],-problem.inputs.uu(3)*ones(1,2),'k--','LineWidth',2)
%plot([T(1),T(end)],problem.inputs.ul(5)*ones(1,2),'k--')
plot([T(1),T(end)],problem.inputs.uu(4)*ones(1,2),'k--','LineWidth',2)
xlim([0 T(end)+step])
xlabel('Time [h]')
ylabel('Battery SOC (x_{2,t})')
title('Energy of LiFePO4 Battery')
legend( 'Energy-Battery (kWh)',  'Battery-Discharge (kW)',  'Battery-Charge (kW)')
...
legend( 'Load Demand, HP', 'Generated Power, PV + u_d')
grid minor; box on;
...
%stairs(T,sold,'LineWidth',1.5 ) %,'k--'
plot(T,sold,'LineWidth',1.5) %,'g--',
xlim([0 T(end)+step])
xlabel('Time [h]')
ylabel('Power (kW)')
title('Power traded with grid')
legend('Bought power','DR','Sold power')%'Sold power (kW)'
grid minor; box on;
ylim([0 3])
