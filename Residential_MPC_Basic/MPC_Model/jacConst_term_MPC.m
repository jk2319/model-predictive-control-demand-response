...
% battery storage
  %dx(:,3)=vdat.ch_eff*uc-ud./vdat.dis_eff;
...
df.dx{1}=[-(UV*A+rhoAir* B_V*C_air*n_ac)*Lt./C_Build, 0*t ]; % Derivatives of f(x,u,p,t) wrt. x1
...
df.du{1}=[Lt.*COPTA./C_Build, 0*t];  % Derivative of f(x,u,p,t) wrt. u1
%df.du{2}=[Lt*Conv_S_S./C_Build, -Conv_S_S*Lt,    0*t];  % Derivative of f(x,u,p,t) wrt. u2
df.du{2}=[-m_cop_cool*Lt./C_Build, 0*t];  % Derivative of f(x,u,p,t) wrt. u3
df.du{3}=[0*t, -Lt./data.data.dis_eff];  % Derivative of f(x,u,p,t) wrt. u4
df.du{4}=[0*t,  data.data.ch_eff*Lt];  % Derivative of f(x,u,p,t) wrt. u5
df.du{5}=[0*t,  0*t];  % Derivative of f(x,u,p,t) wrt. u6
df.du{6}=[0*t,  0*t];  % Derivative of f(x,u,p,t) wrt. u7
...
%g_eq=u_B_grid-u_s_grid+ud-uc+PV_el-u1-u2-u3;
%dg.dx{1}
