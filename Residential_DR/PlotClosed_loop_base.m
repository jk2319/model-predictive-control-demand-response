...
legend('Base','Case 3-B','External Temperature') %CASE!!!!!!!!!!!!!
grid minor; box on;
ax = gca;
...
legend( 'Base (kWh)', 'Case 3-B (kWh)', 'Battery-Discharge (kW)',  'Battery-Charge (kW)')
% CASE!!!!!!!!!!
grid minor; box on;
...
subplot(4,1,2)
hold on
plot(T,LoadBase,'k:','LineWidth',2.5)
...
% % Price of electricity
yyaxis left
plot(xx,ct,'LineWidth',1)
%xlim([105 130]) 
xlim([start1 end1])
xlabel('Time [h]','FontSize',18)
ylabel('Price (£/kWh)','FontSize',18)
title('Load Shifting in Demand Profile','FontSize',18)
%legend( '£/h')

yyaxis right
plot(T,LoadBase,'k:','LineWidth',2)
plot(T,Load,'r-','LineWidth',1)
%xlim([105 130]) 
ylabel('Power (kW)','FontSize',18)
%title('Load Demand, HP')
legend( 'Electricity Tariff','Base Case', 'DR Case','FontSize',18) %CASE!!!!!!!!!!
grid minor; box on;
