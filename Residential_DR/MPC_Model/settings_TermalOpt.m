function options = settings_TermalOpt(varargin)
...
options.adigatorPath='/home/pfalugi/software/matlab2017/ICLOCS_2.5_PreRelease/adigator';
...
options.ipopt.mehrotra_algorithm='yes';        % algorithm suitable for LPs 

%options.ipopt.constr_viol_tol=0.030;           % Absolute tolerance on the constraint violation.
                                               % Successful termination requires that the max-norm of the (unscaled) constraint violation is less than this threshold.
                                               % Default value 0.0001 

%options.ipopt.compl_inf_tol=0.01;               %Absolute tolerance on the complementarity.  Default value is 0.0001

%options.ipopt.acceptable_tol=0.03;           %Determines which (scaled) overall optimality error is considered to be "acceptable".
                                                 % Default value is 10-06
%options.ipopt.acceptable_constr_viol_tol=0.05;   % Absolute tolerance on the constraint violation. Default value is 0.01.
...
% Ir removes t0 and tf as decision variables. 

options.runWithoutTimeVar=1;

% External Constraint Handling
%---------------------------------------
% For problems with large sets of inactive constraints, external constraint handling may help to reduce the computation cost
...
%---------------------------------------
% A regularization parameter could be setup in the problem file with problem.data.penalty.values defining the regularization sequence of weights, and problem.data.penalty.i the starting index. 
% The parameter could be called with vdat.penalty.values(vdat.penalty.i) in problem formulation.
