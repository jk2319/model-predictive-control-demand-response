...
%   dE - Gradient of E with respect to tf,p,x0,u0,uf,xf (boundaryCost)
%  
%  Gradient dL:
...
a1=interp1(data.data.T_mex,data.data.alpha1, t,'previous'); %,vdat.alpha1(end));
...
% Derivative with respect to U(:,5) (Bought power)
e5=zeros(1,m);
e5(5)=1;
du5=kron(ct(1:size(t,1))+Price_carbon*c_em(1:size(t,1)).*delt,e5);
%du6=kron(ct,e6);

% Derivative with respect to U(:,6) (Sold power)
e6=zeros(1,m);
e6(6)=1;
du6=kron(-0.98*ct(1:size(t,1)),e6);

% Derivative with respect to U(:,8) (Slack variable)
e8=zeros(1,m);
e8(8)=1;
du8=kron(0.1,e8);

% Derivative with respect to U(:,9) (DR power)
e9=zeros(1,m);
e9(9)=1;
du9=kron(-a1(1:size(t,1)).*(0.2),e9);

dL.flag=1; 
dL.dp=[];
dL.dt=[];
dL.dx=kron(zeros(1,n),Lt);
dL.du=du5+du6+du8+du9;
